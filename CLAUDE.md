...
- **2026-08-23 — Owner registration portal.** Multi-step host onboarding (Airbnb-style, Bengaluru-only) at `/owners`, plus login and a pending-listing dashboard. New homes stay `pending_review` / `active: false`. Cookie auth is interim until Supabase. Bank details collected only for later manual payouts.
- **2026-08-23 — Pixel map listing.** Replaced the card grid on `/` with an interactive pixel neighbourhood map. Houses stay on the map when booked (greyed). Same Next.js stack — no new framework.
- **2026-08-23 — Restore card listing.** Guest `/` is the original card grid again; unused pixel-map files and CSS were removed.
- **2026-08-23 — Vercel + Supabase.** Production env vars are on the Vercel project (`DATABASE_URL`, `DIRECT_URL`, `OWNER_SESSION_SECRET`). Prisma/health also accept the connector’s `POSTGRES_*` names. Reset the leaked DB password after the listing loads and update Vercel.
- **2026-08-24 — Platform admin.** `/admin` for listing review (approve / reject / unpublish), bookings (view + mark completed after checkout), owners, and manual 80% payouts. Admin login is a password cookie (`ADMIN_PASSWORD`). Ledger amounts stay immutable; the trigger now allows flipping `payout_status` only. No refunds/cancellations. Set `ADMIN_PASSWORD` on Vercel before using this in production.
- **2026-08-26 — Committed + pushed the admin build.** The 2026-08-24 admin work (this whole file's entry above) was built but left uncommitted in the working tree, along with a small supporting diff (`ADMIN_PASSWORD` in `.env.example`, a `hasAdminPassword` health-check flag, a `formatDateOnly` helper). Verified `npm run build` passes clean (all admin/owner routes render, all dynamic — no static DB pre-render), confirmed the guest listing (`src/lib/availability.ts`) already filters to `active: true, listingStatus: "active"` so pending/rejected properties stay hidden, and confirmed the ledger migration only widens the trigger for `payout_status` — every money column stays blocked. Committed (`179a166`) and pushed to `origin/main`. **Not yet done — needs a human or cloud-DB access:** (1) the `20260824120000_ledger_allow_payout_status` migration has not been applied to the Supabase prod DB yet — local `.env` still points at the Docker/localhost DB, nobody in this session has the real Supabase connection string, so run `npm run db:migrate` against prod (or via a Vercel deploy hook) before `/admin` payout-flip works in production; (2) `ADMIN_PASSWORD` needs to be set on Vercel (per the 08-24 note) or `/admin` login will fail live; (3) could not confirm the live Vercel deploy actually succeeded post-push — no Vercel CLI/dashboard access or known production URL in this session, someone with Vercel access should confirm the build went green.
